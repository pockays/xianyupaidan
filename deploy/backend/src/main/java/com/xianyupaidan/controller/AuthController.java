...
import com.xianyupaidan.dto.OAuthCallbackRequest;
...
public class AuthController {
    ...
    public Result<Map<String, String>> getOAuthUrl(@RequestBody Map<String, String> body) {
        return Result.ok(authService.getOAuthAuthorizeUrl(body.getOrDefault("state", "user")));
    }

    @PostMapping("/oauth/user-login")
    public Result<LoginResponse> oauthUserLogin(@Valid @RequestBody OAuthCallbackRequest request) {
        return Result.ok(authService.oauthUserLogin(request.getCode(), request.getState(), request.getSellerId()));
    }

    @PostMapping("/oauth/admin-login")
    public Result<LoginResponse> oauthAdminLogin(@Valid @RequestBody OAuthCallbackRequest request) {
        return Result.ok(authService.oauthAdminLogin(request.getCode(), request.getState()));
    }
