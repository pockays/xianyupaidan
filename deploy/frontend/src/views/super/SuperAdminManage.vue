      ...
      <table class="data-table" v-if="admins.length">
        ...
        <tbody>
          <tr v-for="admin in admins" :key="admin.id" :class="{ disabled: admin.status === 0 }">
            ...
            <td class="col-xianyu">{{ (admin as any).xianyuId || '-' }}</td>
    ...
    <transition name="fade">
      <div v-if="showDialog" class="overlay">
        <div class="dialog">
          ...
          <div class="form-group">
            <label class="form-label">闲鱼ID <span class="optional">管理员授权登录用</span></label>
            <input v-model="form.xianyuId" class="form-input" placeholder="绑定管理员的闲鱼账号ID" />
          </div>
          <div class="form-group">
...
const form = ref({ username: '', password: '', email: '', xianyuId: '' })

onMounted(async () => { admins.value = await getAdmins() })

function openCreate() { isEdit.value = false; editingId.value = 0; form.value = { username: '', password: '', email: '', xianyuId: '' }; showPwd.value = false; showDialog.value = true }
function openEdit(row: AdminInfo) { isEdit.value = true; editingId.value = row.id; form.value = { username: row.username, password: '', email: row.email || '', xianyuId: (row as any).xianyuId || '' }; showPwd.value = false; showDialog.value = true }
...
async function handleSave() {
  ...
  if (isEdit.value) {
    await updateAdmin(editingId.value, { username: form.value.username, password: form.value.password || undefined, email: form.value.email, xianyuId: form.value.xianyuId })
