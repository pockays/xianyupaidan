<template>
  <div class="login-page">
    ...
    <div class="login-container">
      ...
      <div class="mode-tabs">
        <button class="mode-tab" :class="{ active: tab === 'user' }" @click="tab = 'user'">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
          用户登录
        </button>
        <button class="mode-tab" :class="{ active: tab === 'admin' }" @click="tab = 'admin'">
        ...
        <button class="mode-tab" :class="{ active: tab === 'super' }" @click="tab = 'super'">
      ...
      <div class="login-card">
        <transition name="card-swap" mode="out-in">
          <div v-if="tab === 'user'" key="user" class="login-form">
            <p class="oauth-hint">通过闲鱼账号授权登录</p>
            <button class="btn-oauth" :disabled="loading" @click="handleOAuth('user')">
              <span v-if="loading" class="spinner"></span>
              <svg v-else width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M8 3v18"/></svg>
              <span>闲鱼授权登录</span>
            </button>
          </div>

          <div v-else-if="tab === 'admin'" key="admin" class="login-form">
            <p class="oauth-hint">通过闲鱼账号授权登录<br/>（需由超管预先绑定闲鱼ID）</p>
            <button class="btn-oauth" :disabled="loading" @click="handleOAuth('admin')">
              <span v-if="loading" class="spinner"></span>
              <svg v-else width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
              <span>闲鱼授权登录</span>
            </button>
          </div>

          <div v-else key="super" class="login-form">
            <div class="form-group">
              <label class="form-label">用户名</label>
              <input type="text" class="form-input" v-model="superForm.username" placeholder="输入超级管理员用户名" />
            </div>
            <div class="form-group">
              <label class="form-label">密码</label>
              <div class="input-wrap">
                <input :type="showPwd ? 'text' : 'password'" class="form-input" v-model="superForm.password" placeholder="输入密码" @keyup.enter="handleSuperLogin" />
            ...
            <button class="btn-primary" :disabled="loading" @click="handleSuperLogin">
              <span v-if="loading" class="spinner"></span>
              <span v-else>登录</span>
            </button>
...
import { adminLogin, getOAuthUrl } from '../../api/auth'
import { useAuthStore } from '../../stores/auth'

const router = useRouter()
const auth = useAuthStore()

const tab = ref<'user' | 'admin' | 'super'>('user')
...
onMounted(() => {
  ...
  if (remember.value) {
    ...
    if (uname) superForm.value.username = uname
  }
})

watch(remember, (v) => {
  localStorage.setItem('login_remember', v ? '1' : '0')
  if (!v) localStorage.removeItem('login_username')
})

async function handleOAuth(type: string) {
  loading.value = true
  try {
    const state = type + '_' + Date.now()
...
async function handleSuperLogin() {
  if (!superForm.value.username || !superForm.value.password) {
    ElMessage.warning('请填写用户名和密码')
    return
  }
  loading.value = true
  try {
    const res = await adminLogin(superForm.value)
    if (res.role !== 'SUPER_ADMIN') {
      ElMessage.error('非超级管理员账号，请使用闲鱼授权登录')
      return
    }
    if (remember.value) {
      localStorage.setItem('login_username', superForm.value.username)
    }
...
.mode-tab {
  ...
  padding: 10px 8px; border-radius: var(--radius-md); border: none;
...
.oauth-hint { text-align: center; color: var(--color-text-secondary); font-size: var(--font-size-sm); line-height: 1.6; margin: 0; }
.btn-oauth {
  width: 100%; padding: 14px 24px; border: none; border-radius: var(--radius-md);
  background: #FF5000; color: #FFF; font-size: var(--font-size-base);
...
.btn-oauth:hover { background: #E04800; box-shadow: 0 4px 12px rgba(255,80,0,0.4); transform: translateY(-1px); }
